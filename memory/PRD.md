...
- **Badge = propositions NON LUES** (2026-08-08) : les propositions ont un champ `read`. `GET /admin/proposals` renvoie `read` par item + `unread`. Nouvel endpoint `POST /admin/proposals/{id}/read`. Le badge du bouton « Proposer » affiche le nombre de non-lues ; cliquer sur une proposition la marque comme lue (elle sort du compteur) tout en ouvrant le formulaire. Indicateur visuel « NOUVEAU » sur les non-lues + « X non lues » dans l'en-tête de la modale. Testé (curl : unread 2→1 après read ; frontend : badge, tag NOUVEAU, clic→lu→badge 0).
- **Refus (DELETE `/admin/proposals/{id}`)** : envoie une notification bienveillante à l'auteur (« À propos de votre proposition ») avant suppression.
